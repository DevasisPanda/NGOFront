...
// Module-level static data — created once, not on every render
const DEFAULT_INTRO = [
  {
  ...
  if (isLoading) {
    return (
      <div className="flex h-screen w-full items-center justify-center bg-white">
        <span className="animate-spin inline-block w-8 h-8 border-4 border-t-[#e67e22] border-r-transparent border-b-transparent border-l-transparent rounded-full" />
      </div>
    );
  }

  // Fallbacks matching the original static content
  const quote = settings?.quote || "Every human life deserves dignity, hope, opportunity, and a future.";
  const motto = settings?.motto || "Service to Humanity is Service to God.";
  const trustName = settings?.trustName || "Valmiki Samaj Charitable Trust";
  const regNo = settings?.regNo || "F/1968/Aravalli";
  const established = settings?.established || "24 January 2020";
      ...
      <section className="bg-primary text-white py-20 relative overflow-hidden">
        ...
        <div className="container-main text-center relative z-10">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 bg-white/10 backdrop-blur-sm px-4 sm:px-6 py-3 rounded-2xl sm:rounded-full border border-white/20 shadow-lg text-sm md:text-base mx-auto w-fit max-w-full">
             <span className="text-secondary font-bold">Our Motto:</span> <span>"{motto}"</span>
          </div>
        </div>
      </section>
        ...
        <div className="container-main">
          ...
          <div className="mb-24 fade-in-section opacity-0 translate-y-5 transition-all duration-800 ease-out [&.is-visible]:opacity-100 [&.is-visible]:translate-y-0">
            <div className="flex flex-col lg:flex-row gap-16 items-start">
              <div className="flex-1 order-2 lg:order-1">
                <h2 className="text-[28px] md:text-[36px] font-bold text-primary mb-8 border-l-4 border-secondary pl-4 leading-tight">{trustName}</h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-primary font-bold text-[15px]">{regNo}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-primary font-bold text-[15px]">{established}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 sm:col-span-2 mt-2 pt-6 border-t border-gray-200">
                      ...
                      <div>
                        ...
                        <p className="text-primary font-bold text-xl">{founder}</p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-6 text-[#45464e] text-[17px] leading-relaxed">
                  {introParagraphs.map((para, index) => {
                    if (para.isBoldSecondary) {
                      return (
                        <p key={index} className="text-secondary font-bold text-lg whitespace-pre-line">
                          {para.text}
                        </p>
                      );
                    }
                    return (
                      <p key={index} className="whitespace-pre-line">
                        {para.boldPrefix && (
                          <strong className="text-primary mr-1">{para.boldPrefix}</strong>
                        )}
                        {para.text}
                      </p>
                    );
              ...
              <div className="lg:w-[450px] shrink-0 order-1 lg:order-2 lg:sticky lg:top-8 w-full">
                <div className="relative group mx-auto max-w-[280px] sm:max-w-[350px] lg:max-w-none">
                  ...
                  <img alt="Valmiki Samaj Trust Logo" className="w-full h-auto rounded-[2rem] shadow-2xl border-4 sm:border-8 border-white relative z-10 bg-white p-6 sm:p-8 md:p-12 object-contain aspect-square" src={logoUrl} />
          ...
          <div className="mb-24 fade-in-section opacity-0 translate-y-5 transition-all duration-800 ease-out [&.is-visible]:opacity-100 [&.is-visible]:translate-y-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
              {commitments.map((card, index) => (
                <div key={index} className="bg-[#f8f9fa] p-8 md:p-10 rounded-3xl hover:shadow-[0_10px_40px_rgb(0,0,0,0.08)] transition-all duration-300 hover:-translate-y-1 border-t-4 border-transparent hover:border-secondary group">
                  ...
                  <h3 className="text-[24px] font-bold text-primary mb-4">{card.title}</h3>
                  <p className="text-[#45464e] leading-relaxed text-[16px] whitespace-pre-line">
                    {card.description}
                  </p>
                </div>
              ))}
          ...
          <div className="mb-24 bg-primary rounded-[2.5rem] p-6 sm:p-8 md:p-14 lg:p-16 text-white relative overflow-hidden fade-in-section opacity-0 translate-y-5 transition-all duration-800 ease-out [&.is-visible]:opacity-100 [&.is-visible]:translate-y-0 shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row gap-16 items-center">
              <div className="lg:w-1/3">
                <h2 className="text-[28px] sm:text-[36px] md:text-[46px] font-bold text-white mb-6 leading-tight">{visionTitle}</h2>
                <div className="w-20 h-1.5 bg-secondary mb-8 rounded-full"></div>
                <p className="text-gray-300 text-[18px] leading-relaxed whitespace-pre-line">
                  {visionDescription}
                </p>
              </div>
          ...
          <div className="mb-24 fade-in-section opacity-0 translate-y-5 transition-all duration-800 ease-out [&.is-visible]:opacity-100 [&.is-visible]:translate-y-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
              {coreValues.map((val, index) => (
                <div key={index} className="bg-[#f8f9fa] p-6 sm:p-8 rounded-2xl text-center border-b-4 border-secondary hover:-translate-y-2 transition-all duration-300 hover:shadow-xl group">
                  <div className="bg-white w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm group-hover:scale-110 transition-transform">
                    <span className="material-symbols-outlined text-3xl text-primary">{val.icon}</span>
                  </div>
                  <h3 className="text-[20px] font-bold text-primary">{val.title}</h3>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#f8f9fa] rounded-[2.5rem] p-6 sm:p-8 md:p-16 border border-gray-100 fade-in-section opacity-0 translate-y-5 transition-all duration-800 ease-out [&.is-visible]:opacity-100 [&.is-visible]:translate-y-0 shadow-sm relative overflow-hidden">
            ...
            <div className="max-w-4xl mx-auto text-center relative z-10">
              ...
              <h2 className="text-[28px] md:text-[36px] font-bold text-primary mb-8">{promiseTitle}</h2>
              <p className="text-[22px] md:text-[26px] text-primary leading-relaxed mb-12 italic font-medium px-4 whitespace-pre-line">
                "{promiseText}"
              </p>
              
              <div className="h-px w-full max-w-2xl mx-auto bg-gradient-to-r from-transparent via-gray-300 to-transparent mb-12"></div>
              
              <h3 className="text-[24px] md:text-[30px] font-bold text-primary mb-6">{joinTitle}</h3>
              <p className="text-muted text-[18px] mb-12 leading-relaxed px-4 md:px-12 whitespace-pre-line">
                {joinDescription}
              </p>
